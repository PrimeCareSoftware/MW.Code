# Guia do Usuário - Sistema de Telemedicina

## Índice
1. [Introdução](#introdução)
2. [Regulamentação e Conformidade](#regulamentação-e-conformidade)
3. [Requisitos do Sistema](#requisitos-do-sistema)
4. [Primeiros Passos](#primeiros-passos)
5. [Criando Sessões de Telemedicina](#criando-sessões-de-telemedicina)
6. [Processo de Consentimento](#processo-de-consentimento)
7. [Iniciando e Entrando em Videochamadas](#iniciando-e-entrando-em-videochamadas)
8. [Durante a Consulta](#durante-a-consulta)
9. [Finalizando Sessões](#finalizando-sessões)
10. [Histórico de Sessões](#histórico-de-sessões)
11. [Solução de Problemas](#solução-de-problemas)
12. [Privacidade e Proteção de Dados](#privacidade-e-proteção-de-dados)
13. [Boas Práticas](#boas-práticas)

---

## Introdução

O módulo de Telemedicina do Omni Care permite que profissionais de saúde realizem consultas, acompanhamentos e orientações médicas à distância, utilizando tecnologia de videoconferência segura e integrada ao prontuário eletrônico.

### O que é Telemedicina?

A telemedicina é a prática médica mediada por tecnologia para fins de:
- **Teleconsulta**: Consulta médica remota
- **Teleinterconsulta**: Troca de informações entre profissionais
- **Telemonitoramento**: Acompanhamento de parâmetros de saúde
- **Teleorientação**: Orientações e esclarecimentos

### Benefícios

✅ **Para Profissionais:**
- Ampliação do acesso aos serviços
- Redução de deslocamentos
- Otimização da agenda
- Atendimento em áreas remotas
- Continuidade do cuidado

✅ **Para Pacientes:**
- Conveniência e conforto
- Economia de tempo e custos
- Acesso facilitado a especialistas
- Redução de exposição (importante em pandemias)
- Acompanhamento mais frequente

---

## Regulamentação e Conformidade

### Resolução CFM 1.643/2002 e CFM 2.314/2022

A telemedicina no Brasil é regulamentada pelo Conselho Federal de Medicina através das resoluções:

**Princípios Fundamentais:**
- Telemedicina é ato médico regulamentado
- Deve seguir padrões éticos da profissão médica
- Requer registro de dados em prontuário
- Necessita consentimento informado do paciente
- Exige infraestrutura tecnológica adequada

### Quando Utilizar Telemedicina

✅ **Permitido:**
- Consultas de retorno e acompanhamento
- Renovação de receitas de uso contínuo
- Orientações e esclarecimentos
- Avaliação de resultados de exames
- Apoio assistencial
- Interconsultas entre profissionais
- Triagem e avaliação inicial

⚠️ **Com Restrições:**
- Primeira consulta (recomendado presencial, mas permitido remotamente)
- Atestados e laudos (verificar exigências específicas)
- Prescrição de medicamentos controlados (seguir legislação)

❌ **Não Recomendado:**
- Emergências médicas (orientar buscar atendimento presencial)
- Situações que exigem exame físico detalhado
- Procedimentos invasivos
- Quando tecnologia não permite avaliação adequada

### Obrigações Legais

📋 **O profissional deve:**
1. Possuir registro ativo no CRM
2. Informar número do CRM e UF ao paciente
3. Obter consentimento livre e esclarecido
4. Registrar atendimento em prontuário
5. Manter sigilo e segurança dos dados
6. Fornecer receitas e atestados digitalmente assinados
7. Disponibilizar identificação clara na interface

---

## Requisitos do Sistema

### Para Profissionais de Saúde

**Hardware Mínimo:**
- Computador ou notebook com:
  - Processador: Intel Core i3 ou equivalente
  - Memória RAM: 4 GB (recomendado 8 GB)
  - Webcam HD (720p mínimo, 1080p recomendado)
  - Microfone (headset recomendado para melhor qualidade)
  - Caixas de som ou fones de ouvido

**Software:**
- Sistema Operacional:
  - Windows 10/11
  - macOS 10.14 ou superior
  - Linux (distribuições recentes)
- Navegador atualizado:
  - Google Chrome 90+ (recomendado)
  - Mozilla Firefox 88+
  - Microsoft Edge 90+
  - Safari 14+

**Conexão de Internet:**
- Velocidade mínima: 2 Mbps download / 1 Mbps upload
- Recomendado: 5 Mbps download / 2 Mbps upload
- Conexão estável (WiFi ou cabo)

### Para Pacientes

**Dispositivos Compatíveis:**
- Computador/Notebook (mesmo requisitos acima)
- Smartphone:
  - Android 8.0 ou superior
  - iOS 12 ou superior
- Tablet (Android ou iPad)

**Aplicativo:**
- Não é necessário instalar aplicativos
- Acesso via navegador web
- Link direto enviado por SMS/E-mail

**Conexão de Internet:**
- Velocidade mínima: 1 Mbps
- WiFi ou dados móveis 4G/5G

---

## Primeiros Passos

### Configuração Inicial do Profissional

#### Passo 1: Configurar Perfil de Telemedicina

1. Acesse **Menu** → **Meu Perfil** → **Telemedicina**
2. Complete as informações:
   - **Número do CRM**: Seu registro profissional
   - **UF do CRM**: Estado de registro
   - **Especialidades**: Selecione suas especialidades
   - **Foto Profissional**: Upload de foto (aparecerá na videochamada)

3. **Configurar Disponibilidade:**
   - Dias da semana disponíveis para telemedicina
   - Horários de atendimento
   - Duração padrão das consultas (30, 45, 60 minutos)

---

## Criando Sessões de Telemedicina

### Métodos de Criação

#### 1. Através da Agenda

1. Acesse **Agenda** no menu principal
2. Clique no horário desejado
3. Selecione **"Agendar Teleconsulta"**
4. Preencha os dados do paciente e tipo de consulta
5. Clique em **"Agendar"**

✅ O sistema automaticamente:
- Cria a sessão de telemedicina
- Envia confirmação ao paciente
- Envia link de acesso 30 minutos antes

---

## Processo de Consentimento

### Importância do Consentimento

O consentimento informado é **obrigatório** conforme:
- Resolução CFM 2.314/2022
- Lei Geral de Proteção de Dados (LGPD)
- Código de Ética Médica

### Como Funciona

1. Paciente acessa o link da consulta
2. Sistema exibe termo de consentimento
3. Paciente aceita os termos
4. Somente após aceite, pode entrar na sala

---

## Iniciando e Entrando em Videochamadas

### Para o Profissional

1. Na **Agenda**, clique na consulta agendada
2. Botão **"Entrar na Sala"** aparece 15 minutos antes
3. Clique para acessar
4. Aguarde o paciente na sala de espera
5. Clique em **"Admitir Paciente"** quando ele entrar

### Para o Paciente

1. Clicar no link recebido por SMS/E-mail
2. Autorizar câmera e microfone no navegador
3. Aceitar termo de consentimento
4. Entrar na sala de espera
5. Aguardar admissão pelo profissional

---

## Durante a Consulta

### Controles Disponíveis

🎤 **Microfone** - Mutar/Desmutar (Atalho: M)
📹 **Câmera** - Ligar/Desligar vídeo (Atalho: V)
🔊 **Alto-falante** - Ajustar volume
🖥️ **Compartilhar Tela** - Mostrar documentos/exames
💬 **Chat** - Mensagens de texto
📸 **Captura de Tela** - Documentar visualmente
📝 **Prontuário** - Acesso durante chamada
🚪 **Encerrar Consulta** - Finalizar

### Recursos Principais

#### 1. Prontuário Integrado
- Acesso ao histórico do paciente
- Preenchimento em tempo real
- Consulta de exames anteriores

#### 2. Prescrição Digital
- Prescrever medicamentos durante a consulta
- Assinar digitalmente (se certificado configurado)
- Enviar automaticamente ao paciente

#### 3. Solicitação de Exames
- Solicitar exames necessários
- Gerar guias/pedidos
- Enviar ao paciente

#### 4. Atestado Médico
- Emitir atestados durante consulta
- Assinar digitalmente
- Enviar ao paciente

---

## Finalizando Sessões

### Encerrando a Videochamada

1. Resuma orientações principais
2. Defina próximos passos
3. Clique em **"Encerrar Consulta"**
4. Preencha a evolução médica
5. Marque: ☑️ "Atendimento via Telemedicina"
6. Salvar evolução

---

## Histórico de Sessões

### Consultar Sessões Realizadas

1. Acesse **Telemedicina** → **Histórico**
2. Utilize filtros:
   - Período
   - Paciente
   - Status (Concluídas, Canceladas)
   - Tipo de consulta

### Estatísticas Pessoais

- Total de teleconsultas no mês
- Tempo médio de consulta
- Taxa de comparecimento
- Avaliação média dos pacientes

---

## Solução de Problemas

### Problemas de Áudio

**Não consigo ouvir o paciente:**
1. Verifique volume do computador
2. Selecione dispositivo de saída correto
3. Atualize a página e reconecte

**Paciente não me ouve:**
1. Verifique se microfone não está mudo
2. Confirme permissões do navegador
3. Teste com outro aplicativo

### Problemas de Vídeo

**Vídeo não aparece:**
1. Verifique se câmera não está em uso por outro app
2. Recarregue a página
3. Reinicie o navegador

**Vídeo travando:**
1. Reduza qualidade do vídeo
2. Feche outras abas/aplicativos
3. Verifique conexão de internet

### Problemas de Conexão

**Não consigo entrar na sala:**
1. Verifique se link está correto
2. Tente navegador diferente
3. Tente modo anônimo/privativo
4. Desabilite extensões temporariamente

**Conexão cai durante consulta:**
1. Paciente deve reentrar pelo mesmo link
2. Profissional aguarda na sala
3. Se não reconectar em 5 min, entre em contato por telefone

---

## Privacidade e Proteção de Dados

### Conformidade com a LGPD

O sistema está em conformidade com a Lei 13.709/2018:

**Segurança:**
- Criptografia de ponta a ponta (E2E)
- Protocolo DTLS-SRTP
- Padrão AES-256
- Servidores certificados ISO 27001

**Direitos do Paciente:**
- Acesso aos dados
- Correção de informações
- Exclusão (com ressalvas legais)
- Portabilidade
- Revogação de consentimento

### Retenção de Dados

- **Prontuário Médico**: 20 anos (CFM 1.821/2007)
- **Gravações**: 5 anos
- **Logs de Acesso**: 6 meses
- **Consentimentos**: Enquanto houver relação

---

## Boas Práticas

### Preparação do Ambiente

**Iluminação:**
- Luz natural de frente
- Iluminação frontal adequada
- Evitar sombras no rosto

**Fundo:**
- Parede neutra ou estante organizada
- Evitar janelas com movimento
- Opcional: fundo virtual

**Áudio:**
- Ambiente silencioso
- Fechar portas e janelas
- Usar headset para melhor qualidade

### Durante o Atendimento

**Faça:**
- Mantenha contato visual (olhe para câmera)
- Vista-se adequadamente
- Demonstre empatia e atenção
- Fale clara e pausadamente

**Evite:**
- Atender em ambientes inadequados
- Comer ou beber durante consulta
- Realizar múltiplas tarefas
- Interromper o paciente

### Documentação

**Registre sempre:**
- "Atendimento realizado via telemedicina"
- Consentimento obtido
- Limitações do exame físico remoto
- Qualidade da conexão
- Orientações e condutas

---

## Perguntas Frequentes (FAQ)

**P: A consulta pode ser gravada?**  
R: Sim, se habilitado e com consentimento adicional do paciente.

**P: Funciona em celular?**  
R: Sim, compatível com Android e iOS via navegador.

**P: Precisa instalar programa?**  
R: Não, acesso 100% via navegador web.

**P: É obrigatório certificado digital?**  
R: Não para teleconsultas, mas necessário para assinatura digital de documentos.

**P: Atestado por telemedicina é válido?**  
R: Sim, se assinado digitalmente.

**P: É seguro? Dados podem vazar?**  
R: Sistema usa criptografia de ponta a ponta, mesmo padrão de bancos.

---

## Suporte e Recursos

### Documentação Oficial

📖 **Resoluções CFM:**
- CFM 2.314/2022 - Define telemedicina
- CFM 1.821/2007 - Prontuário eletrônico

📜 **LGPD:**
- Lei 13.709/2018

### Suporte Técnico

**Central de Ajuda:**
- 📧 E-mail: suporte@omnicare.com.br
- 📞 Telefone: (11) 1234-5678
- 💬 Chat: Disponível no sistema
- ☎️ Emergência 24/7: 0800-XXX-XXXX

---

## Checklist de Boas Práticas

### Antes da Consulta
- [ ] Ambiente silencioso preparado
- [ ] Iluminação adequada
- [ ] Câmera e microfone testados
- [ ] Conexão de internet verificada
- [ ] Prontuário do paciente revisado

### Durante a Consulta
- [ ] Apresentação profissional
- [ ] Contato visual mantido
- [ ] Escuta ativa e empática
- [ ] Orientações claras fornecidas
- [ ] Documentos gerados e enviados

### Após a Consulta
- [ ] Evolução preenchida
- [ ] Modalidade "telemedicina" marcada
- [ ] Documentos enviados ao paciente
- [ ] Retorno agendado (se necessário)

---

## Glossário

**AES-256**: Padrão de criptografia avançada  
**DTLS-SRTP**: Protocolo de segurança para chamadas  
**E2E**: End-to-End (ponta a ponta)  
**LGPD**: Lei Geral de Proteção de Dados  
**Teleconsulta**: Consulta médica remota  
**Teleinterconsulta**: Consulta entre profissionais

---

**Versão do documento**: 1.0  
**Última atualização**: Janeiro 2025  
**Sistema**: Omni Care v2.0

---

💡 **Lembre-se**: A telemedicina é uma ferramenta poderosa para ampliar o acesso à saúde. Use-a com responsabilidade, ética e sempre priorizando o bem-estar do paciente.

🏥 **Boas Teleconsultas!**
