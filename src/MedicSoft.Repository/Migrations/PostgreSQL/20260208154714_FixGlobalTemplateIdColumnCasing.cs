...
{
    /// <inheritdoc />
    public partial class FixGlobalTemplateIdColumnCasing : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Fix GlobalTemplateId column casing if it exists with lowercase name
            // This handles the case where the column was created as lowercase 'globaltemplateid'
            // but EF Core expects it as PascalCase 'GlobalTemplateId'
            migrationBuilder.Sql(@"
                DO $$ 
                BEGIN
                    -- Check if lowercase column exists and PascalCase doesn't
                    IF EXISTS (
                        SELECT 1 
                        ...
                        -- Rename the column to PascalCase
                        ALTER TABLE ""DocumentTemplates"" RENAME COLUMN globaltemplateid TO ""GlobalTemplateId"";
            ...
            // Only alter CustomDashboards if the table and columns exist
            migrationBuilder.Sql(@"
                DO $$
                BEGIN
                    IF EXISTS (
        ...
        {
            // Rollback: Rename column back to lowercase if PascalCase version exists
            migrationBuilder.Sql(@"
                DO $$ 
                BEGIN
                    -- Check if PascalCase column exists and lowercase doesn't
                    IF EXISTS (
                        SELECT 1 
                        FROM information_schema.columns 
                        WHERE table_name = 'documenttemplates'
                        ...
                        -- Rename the column back to lowercase
                        ALTER TABLE ""DocumentTemplates"" RENAME COLUMN ""GlobalTemplateId"" TO globaltemplateid;
            ...
            // Only alter DashboardWidgets if the table and columns exist (rollback)
            migrationBuilder.Sql(@"
                DO $$
                BEGIN
                    IF EXISTS (
            ...
            // Only alter CustomDashboards if the table and columns exist (rollback)
            migrationBuilder.Sql(@"
                DO $$
                BEGIN
                    IF EXISTS (
