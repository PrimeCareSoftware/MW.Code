...
import { TipoDocumento, ResultadoAssinatura, AssinarDocumentoRequest } from '../../models/assinatura-digital.model';
...
export class AssinarDocumentoComponent implements OnInit {
  ...
  carregarCertificados(): void {
    ...
    this.certificadoService.listarCertificados().subscribe({
      next: (data: CertificadoDigital[]) => {
        const certificadosValidos = data.filter((c: CertificadoDigital) => c.valido);
  ...
  assinar(): void {
    ...
    this.assinaturaService.assinarDocumento(request).subscribe({
      next: (resultado: ResultadoAssinatura) => {
